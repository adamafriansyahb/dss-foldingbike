...
class Design extends CI_Controller
{
    ...
    {
        $data['title'] = 'Compare the design of the bikes';
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('design/design_input', $data);
            ...
            $query = $this->db->get_where('design_comparison', array('id_user' => $user_id));

            if ($query->num_rows() < 1) {
                $this->db->insert_batch('design_comparison', $data);
            } else {
                $delete_query = "DELETE FROM `design_comparison` WHERE `id_user` = $user_id";
                $this->db->query($delete_query);
                $this->db->insert_batch('design_comparison', $data);
            }

            redirect('design/comparison_result');
        }
    }

    public function comparison_result()
    {
        $data['title'] = 'Design Comparisons Result';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        ...
        $query_comparison = "SELECT * FROM `design_comparison` WHERE `id_user`='$user_id' ORDER BY `id` ASC";
        ...
        $query_a1 = "SELECT SUM(`value`) AS 'sum' from `design_comparison` WHERE `alternative_2`= '$alternative_1' AND `id_user`= '$user_id'";
        $query_a2 = "SELECT SUM(`value`) AS 'sum' from `design_comparison` WHERE `alternative_2`= '$alternative_2' AND `id_user`= '$user_id'";
        $query_a3 = "SELECT SUM(`value`) AS 'sum' from `design_comparison` WHERE `alternative_2`= '$alternative_3' AND `id_user`= '$user_id'";
        $query_a4 = "SELECT SUM(`value`) AS 'sum' from `design_comparison` WHERE `alternative_2`= '$alternative_4' AND `id_user`= '$user_id'";
        $query_a5 = "SELECT SUM(`value`) AS 'sum' from `design_comparison` WHERE `alternative_2`= '$alternative_5' AND `id_user`= '$user_id'";
        ...
        if ($sum_a1 == null or $sum_a2 == null or $sum_a3 == null or $sum_a4 == null or $sum_a5 == null) {
            ...
            $design_comparison = $this->db->get_where('design_comparison', array('id_user' => $user_id))->result_array();
            ...
            for ($i = 0; $i < sizeof($data['alternatives']); $i++) {
                $data['normalized_a1'][] = array(
                    'id' => $this->db->get_where('design_comparison', array(
                        'id_user' => $user_id,
                        'alternative_1' => $alternatives[0]['name'],
            ...
            // CALCULATING CONSISTENCY RATIO, 1.12 is the RC where number of element is 5.
            ...
            for ($i = 0; $i < 5; $i++) {
                $design_weight[] = array(
                    ...
                    'design' => $data['weight'][$i]
                );
            }

            if ($consistency_ratio >= 0 and $consistency_ratio <= (1 / 10)) {
                $this->db->update_batch('alternatives_selected', $design_weight, 'id');
            }
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('design/design_result', $data);
            $this->load->view('templates/footer');
        }
    }
    ...
    public function error()
    {
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('design/error', $data);
        $this->load->view('templates/footer');
    }
}
